 ...
 */
public class FlowValidationTrackingFlowRenderer implements FlowRenderer<FlowValidationTracking> {

    public static final FlowValidationTrackingFlowRenderer INSTANCE = new FlowValidationTrackingFlowRenderer();
    @Override
    ...
    @Override
    public <W extends SerializationWriter> W toSerialization(W serializationWriter, FlowValidationTracking flowValidationTracking) {
        serializationWriter.object();
        serializationWriter.key("key").value(flowValidationTracking.getMessageKey());
        if ( flowValidationTracking.getMessageParameters() != null ) {
            serializationWriter.key("parameters");
            serializationWriter.array();
            for(Object parameter: flowValidationTracking.getMessageParameters()) {
                serializationWriter.value(parameter);
            }
            serializationWriter.endArray();
        }
        return serializationWriter.endObject();
    }
